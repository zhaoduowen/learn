...
class M_teacher_course extends Base_Model
{
    function __construct()
    {
        parent::__construct();
        $this->table = "teacher_course_master";
    }
 ...
 //根据排课时间获取老师
    ...
    public function getTimetable($course_id){
        return $this->db->get_where('teacher_timetable',array('status'=>1,'course_id' => $course_id))->result_array();
    }


    public  function getOne($where){
       return $this->db->get_where('teacher_course_master',$where)->result_array(); 
    ...
    public function getList($where = array(), $limit = '', $offset = '', $type = 0)
    {
        ...
        $date = date("Y-m-d");
        $sql = " select " . $condition . " from teacher_course_master a 
        ...
        if($type == 1){
            $sql .= " order by a.course_date asc limit {$limit},{$offset}";
            $data = $this->db->query($sql)->result_array();
            foreach ($data as $key => $value) {
                $data[$key]['timetable'] = $this->getTimetable($value['course_id']);
            }
