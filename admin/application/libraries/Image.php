...
// +----------------------------------------------------------------------
// | Author: vic ming <282403156@qq.com>
// +----------------------------------------------------------------------
 ...
 * @package  ORG
 * @subpackage  Util
 * @author    vic ming <282403156@gmail.com>
 */
class Image {
	 ...
	 * @param string $comperssion  图片压缩比★
	 * @param string $position  水印的位置★
     * @return void
     */
    static public function water($source, $water, $savename=null, $alpha=80, $comperssion=80,$position=9,$tcomperssion=85) {
        //检查文件是否存在
		...
		*/
		...
		//保存图像,如果是jpg,则设置一下水印质量(自己添加的)
		if ($sInfo['type'] == 'jpg' || $sInfo['type'] == 'jpeg') {
		imagejpeg($sImage, $savename,$comperssion);//第3个参数即使质量大小，因为只有imagejpeg支持这个参数
		} else {
	 ...
	 * @param string $tcomperssion 缩略图压缩比★
     * @return void
     */
    static function thumb($image,$type='',$thumbname, $maxWidth=200, $maxHeight=50, $interlace=true, $tcomperssion) {
        // 获取原图信息
        ...
        if ($info !== false) {
           ...
           //★★★★★自己加的缩略图压缩质量	 
			...
			if($tcomperssion){
			 $imageFun($thumbImg, $thumbname,$tcomperssion);
			 }else{
			 $imageFun($thumbImg, $thumbname);
			 }
					 }
			//替代$imageFun($thumbImg, $thumbname);
     ...
     * @return void
      +----------------------------------------------------------
     */
    static function thumb2($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true, $tcomperssion) {
        // 获取原图信息
        $info = Image::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
     ...
     * @return string
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify',$fonttype='') {
        //import('ORG.Util.String');
        //$randval = String::randString($length, $mode,'01LlOo');
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            ImageTTFText($im,20,0,$i * 15 + 20,mt_rand(25, 32),$stringColor,$fonttype,$randval{$i});
        }
        //Image::output($im, $type);
        $vcodeArray['im']=$im;
        $vcodeArray['type']=$type;
        return $vcodeArray;
    }
